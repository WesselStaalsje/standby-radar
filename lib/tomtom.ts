...
function parseFeature(
  ...
  if (type !== 2 || !geometry.length) return null;
...
function chooseSample(point: Point, features: FlowFeature[]): TomTomFlowSample | null {
  ...
  // At a motorway midpoint both carriageways can be close. Within a tight window,
  // use the most congested direction: that is operationally more relevant for standby.
