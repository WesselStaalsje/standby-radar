...
const pointWvkId = (point: RoadMetringPoint) => typeof point.wvkId === "number" && Number.isFinite(point.wvkId) ? point.wvkId : null;
...
export function sensorMetrics(seg: Segment, samples: Map<string, SiteTraffic>, closedLaneCount = 0) {
  ...
  const allRows = directionalSites.map(site => ({ site, sample: samples.get(site.id) }))
    .filter((row): row is { site: MeasurementSite; sample: SiteTraffic } => Boolean(row.sample));
  ...
  const threshold = Math.max(22, speedMad * 3.5);
  const rows = qualityRows.filter(row => centerSpeed === null || row.sample.speedKph === null || rawSpeeds.length < 4 || Math.abs(row.sample.speedKph - centerSpeed) <= threshold);
...
export function segmentMatrix(signals: MatrixSignal[], seg: Segment) {
  const relevant = signals.filter(signal => {
    ...
    const direction = normalizeRoadDirection(signal.carriageway);
    return !seg.direction || !direction || direction === seg.direction;
  });
  ...
  for (const signal of relevant) {
    const direction = normalizeRoadDirection(signal.carriageway);
    if (!direction) unknownDirection += 1;
    const key = `${direction ?? "?"}:${Math.floor((signal.km ?? 0) * 10) / 10}:${signal.lane ?? "?"}`;
  ...
  return {
    ...
    closedLaneCount: new Set(values.filter(value => value.closed && value.lane !== null).map(value => value.lane)).size,
    unknownDirection,
  };
}
...
export function segmentEvents(seg: Segment, events: TrafficEvent[]) {
  const items = events.map(event => ({ event, distance: haversineKm(seg, event) })).filter(({ event, distance }) => {
    ...
    const direction = normalizeRoadDirection(event.direction);
    return !seg.direction || !direction || direction === seg.direction;
  });
  const points = items.map(item => eventImpact(item.event, item.distance) * (item.event.roadRef === seg.road ? 1 : .45)).sort((a, b) => b - a).slice(0, 3);
  return { items, score: Math.min(18, points.reduce((a, b) => a + b, 0)), accidents: items.filter(item => item.event.kind === "accident").length, obstructions: items.filter(item => item.event.kind === "obstruction" || item.event.kind === "closure").length };
}
...
export function parseRwsRestAreas(payload: unknown, points: RoadMetringPoint[]): CandidateLocation[] {
  ...
  for (const [index, feature] of features.entries()) {
    ...
    out.push({ id: `rws-${nearest.point.road}-${nearest.point.km.toFixed(1)}-${index}`, name: featureName(feature.properties ?? {}), address: `${nearest.point.road} ${direction ?? ""} km ${nearest.point.km.toFixed(1)} · officiële RWS-verzorgingsplaats`.replace(/\s+/g, " "), lat: location.lat, lng: location.lng, kind: "service_area", source: "rws", verified: true, road: nearest.point.road, accessKm: nearest.point.km, direction, rayon: null, wvkId: pointWvkId(nearest.point) });
  }
  return out;
}
...
export function parseRwsVildCandidates(payload: unknown, points: RoadMetringPoint[], kind: StandbyLocation["kind"], layerLabel: string): CandidateLocation[] {
  ...
  for (const feature of features) {
    ...
    out.push({ id: `rws-vild-${layerLabel.toLowerCase().replace(/\W+/g, "-")}-${feature.id ?? props.objectid ?? out.length}`, name, address: `${road} ${direction ?? ""} km ${nearest.point.km.toFixed(1)} · Rijkswaterstaat ${layerLabel.toLowerCase()}`.replace(/\s+/g, " "), lat: location.lat, lng: location.lng, kind, source: "rws", verified: true, road, accessKm: nearest.point.km, direction, rayon: null, wvkId: pointWvkId(nearest.point) });
  }
  return out;
}

type OsmElement = { id: number; lat?: number; lon?: number; center?: { lat?: number; lon?: number }; tags?: Record<string, string> };
const osmKind = (tags: Record<string, string>): StandbyLocation["kind"] => tags.carpool === "yes" || tags.park_ride === "yes" ? "carpool" : tags.amenity === "fuel" ? "fuel" : tags.amenity === "parking" ? "parking" : tags.amenity === "restaurant" || tags.amenity === "fast_food" ? "restaurant" : "other";
...
export function parseOsmCandidates(payload: unknown, points: RoadMetringPoint[]): CandidateLocation[] {
  ...
  for (const element of elements) {
    ...
    if (typeof lat !== "number" || typeof lng !== "number" || lat < 51 || lat > 52.45 || lng < 3.9 || lng > 6.65) continue;
    ...
    out.push({ id: `osm-${element.id}`, name: osmName(tags, nearest.point.road, nearest.point.km), address: addressParts.length ? addressParts.join(" ") : `${nearest.point.road} ${direction ?? ""} km ${nearest.point.km.toFixed(1)} · OSM-kandidaat nabij snelweg`.replace(/\s+/g, " "), lat, lng, kind, source: "osm", verified: false, road: nearest.point.road, accessKm: nearest.point.km, direction, rayon: null, wvkId: pointWvkId(nearest.point) });
  }
...
export function chooseDynamicStandby(seg: Segment, locations: CandidateLocation[], alreadyChosen: CandidateLocation[]) {
  const candidates = locations.filter(location => {
    ...
    const direction = normalizeRoadDirection(location.direction);
    return !seg.direction || !direction || direction === seg.direction;
  }).map(location => {
    ...
    return { location: { ...location, routeDistanceKm: Math.round(routeDistanceKm * 10) / 10, routeEtaMinutes: Math.round(routeEtaMinutes * 10) / 10, routeVerified }, roadDistance, routeDistanceKm, routeEtaMinutes, routeVerified, rank: locationQuality(location) - routeDistanceKm * 1.15 - directionPenalty - (reused ? 18 : 0) - (overlap ? 5 : 0) };
